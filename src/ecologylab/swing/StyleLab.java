 ...
 * 
 * This file is hereby placed into the Public Domain. This means anyone is free to do whatever they
...
public class StyleLab
{
	...
	private void displayShapefile() throws Exception
	{
		...
		while(file != null)
		{
			...
			System.out.println(style.getDescription().getAbstract() + "\n\n");
			
			// Add the features and the associated Style object to
			// the MapContext as a new MapLayer
			map.addLayer(featureSource, style);
			file = JFileDataStoreChooser.showOpenFile("shp", file.getParentFile(), (Component)null);
	 ...
	 */
	private Style createStyle(File file, FeatureSource featureSource) throws IOException, TransformerException
	{
		...
		Style style = JSimpleStyleDialog.showDialog(null, schema);
		
		FileWriter fos = new FileWriter(sldPath(file));
		BufferedWriter out = new BufferedWriter(fos);
		
		out.write(getStyleString(style));
		out.close();
		
		return style;
	}

	public File sldPath(File file)
	{
	...
	private String getStyleString(Style style) throws TransformerException
	{
