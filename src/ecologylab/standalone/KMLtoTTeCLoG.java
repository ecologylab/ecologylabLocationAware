...
public class KMLtoTTeCLoG
{
	...
	{
		Kml k = (Kml) ElementState.translateFromXML(new File("/u/ablev/kml/fulltestmap.kml"),
				KMLTranslations.get());
			...
			for (Folder fi : f)
			{
				...
				for (Placemark pi : p)
				{
					...
					else if (pi.getName().equals("base"))
					{
						...
						for (GeoCoordinate gc : gco)
						{
							bases.add(new Ellipse2DDoubleState(gc.getLon(), gc.getLat(), Integer.parseInt(pi
									.getDescription()), Integer.parseInt(pi.getDescription())));
						}
					}
					else if (pi.getName().equals("gps"))
					{
						...
						for (GeoCoordinate gc : gco)
						{
							gpsFields.add(new GPSField(Integer.parseInt(pi.getDescription()), gc.getLon(), gc
									.getLat(), 400, 400));
						}
					}
					else if (pi.getName().equals("wifi"))
					{
						...
						for (GeoCoordinate gc : gco)
						{
							wiFi.add(new Ellipse2DDoubleState(gc.getLon(), gc.getLat(), Integer.parseInt(pi
									.getDescription()), Integer.parseInt(pi.getDescription())));
						}
					}
		...
		{
			p2dd.setLocation(Math.floor((p2dd.x() - NW.x()) * WEsplit), Math.floor((NW.y() - p2dd.y())
					* NSsplit));
		}
		...
		{
			e2dd.setFrame(Math.floor((e2dd.getX() - NW.x()) * WEsplit), Math.floor((NW.y() - e2dd.getY())
					* NSsplit), e2dd.getWidth(), e2dd.getHeight());
		}
		for (GPSField gpf : gpsFields)
		{
			gpf.setFrame(Math.floor(((gpf.getX() - NW.x()) * WEsplit) - 200), Math.floor(((NW.y() - gpf
					.getY()) * NSsplit) - 200), 400.0, 400.0);
		}
		for (Ellipse2DDoubleState e2dd : wiFi)
		{
			for (int i = 1; i < 5; i++)
				wiFiFields.add(new WiFiField(i, Math.floor(((e2dd.getX() - NW.x()) * WEsplit)
						- e2dd.getWidth() / (2 * i)), Math.floor(((NW.y() - e2dd.getY()) * NSsplit)
						- e2dd.getHeight() / (2 * i)), Math.floor(e2dd.getWidth() / i), Math.floor(e2dd
						.getHeight()
						/ i)));
		}
